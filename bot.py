...
#================================================ # @Salom_Coder tarqatdi #@Humo_Hacker kanalida 
#manba Nomalum
define('8501918863:AAE6YCS4j3z0JM9RcpmNXVtk2Kh1qUfABRQ');
...
define('DB_USER', 'BAZA_NOMI');
define('DB_PASS', 'BAZA_PAROLI');
...
if ($connect->connect_error) {
    ...
    echo "<h2 style='color:green;'>✅ SQL baza ulandi!</h2>";
}
...
function joinchat($id){
    ...
    // 1-chi o‘rinda Instagram tugmasi (har doim ❌ deb turadi)
    $instagram = "https://www.instagram.com/Humo_Hacker"; // Instagram linkini shu yerga yozing
    $array['inline_keyboard'][] = [
        ['text' => "❌ Instagram: Obuna bo‘ling", 'url' => $instagram]
    ];
    ...
    if ($kanallar == null) {
        ...
        // Oxirgi tugma: Obuna Bo‘ldim
        $array['inline_keyboard'][] = [
            ['text' => "Obuna Bo’ldim ✅", 'callback_data' => "check"]
        ];

        if ($uns == true) {
            sendMessage($id, "❌ <b>Botdan to'liq foydalanish uchun quyidagi kanallarimizga va Instagram profilga obuna bo'ling!</b>", json_encode($array));
            return false;
        } else {
            return true;
        }
...
$panel = replyKeyboard([
    [['text'=>"📊 Statistika", 'callback_data' => 'statistika'], ['text'=>"🔍 Searching", 'callback_data' => 'searching']],
    [['text'=>"🎬 Kino qo'shish", 'callback_data' => 'kino_qoshish'], ['text'=>"🗑️ Kino o'chirish", 'callback_data' => 'kino_ochirish']],
    [['text'=>"👨‍💼 Adminlar", 'callback_data' => 'adminlar'], ['text'=>"📢 Kanallar", 'callback_data' => 'kanallar']],
], true);
...
[['text'=>"➕ Kanal qoshish"],['text'=>"🗑️ Kanal uzish"]],
[['text'=>"🍿 Kino yuklanadigan kanal"],['text'=>"✉️ Habar yuborish"]],
[['text'=>"➕ Majburiy a'zolik"]],
[['text'=>"◀️ Orqaga"]]
]);
...
$mes=sendPhoto("@$kino",$photo,"<b>🎥 Kino Nomi :
...
else if ($text == "🗑️ Kanal uzish" and in_array($cid,$admin)){
...
else if ($text == "📢 Majburiy a'zolik" and in_array($cid,$admin)){
bot('sendMessage',[
'chat_id'=>$cid,
'text'=>"<b>📢 Majburiy a'zolik kanallari:</b>\n\n". file_get_contents("admin/kanal.txt"),
'parse_mode'=>'html',
'reply_markup'=>$cancel
]);
mysqli_query($connect, "UPDATE `user_id` SET `lastmsg` = 'channels' WHERE `id` = $cid");
...
else if($step == "add-admin" and $cid == $admin){
if(is_numeric($text)=="true"){
if($text != $admin){
file_put_contents("admin/admins.txt", "\n$text", 8);
sendMessage($admin, "✅ <b>$text endi bot admini.</b>", $panel);
mysqli_query($connect, "UPDATE `user_id` SET `step` = '0' WHERE `id` = $cid");
exit();
}else{
...
else if ($text == "🔍 Searching" and in_array($cid,$admin)) {
    sendMessage($cid, "🔎 Qidirilayotgan foydalanuvchi ID raqamini kiriting:", $cancel);
    mysqli_query($connect, "UPDATE `user_id` SET `step` = 'searching' WHERE `id` = $cid");
    exit();
}

else if ($step == "searching" and in_array($cid, $admin)) {
    $searched_id = intval($text);
    $result = mysqli_query($connect, "SELECT * FROM `user_id` WHERE `id` = $searched_id LIMIT 1");
    
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $ban = $row['ban'];
        
        $banStatus = $ban ? "⛔ Blocklangan" : "✅ Blocklanmagan";
        $keyboard = [
        ...
        sendMessage($cid, "<b>🔍 Foydalanuvchi topildi!</b>\n\n👤 <b>ID:</b> <code>$searched_id</code>\n
<b>Status:</b> $banStatus", $reply_markup);
        mysqli_query($connect, "UPDATE `user_id` SET `step` = '0' WHERE `id` = $cid");
    } else {
        sendMessage($cid, "❌ Bu ID bo‘yicha foydalanuvchi topilmadi!", $cancel);
    }
    exit();
}
...
$date = $row['film_date'];
if(!$row){
sendMessage($cid, "❌ $text <b>Bunday kodli kino mavjud emas yoki olib tashlangan!</b>");
exit();
}else{
