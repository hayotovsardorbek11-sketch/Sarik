...
app = Flask(__name__)
app.secret_key = "worm_cinema_telegram_edition_2026"
...
<body>
    ...
    <div id="modal">
        <div class="modal-body">
            ...
            <video id="main-v" controls playsinline autoplay></video>
            <div class="p-20">
    ...
    <script>
        ...
        function sendComm() {
            ...
            fetch('/comment', {
                method:'POST',
                headers:{'Content-Type':'application/x-headers-urlencoded'},
...
@app.route('/login')
def login():
    return f'<body style="background:#000;color:#f00;text-align:center;padding-top:100px;"><h1>{SITE_NAME}</h1><form action="/auth" method="POST"><input name="un" required placeholder="Ism..."><button>KIRISH</button></form></body>'

@app.route('/auth', methods=['POST'])
def auth():
    un = request.form.get('un', '').strip()
    session['user'] = un
    conn = get_db(); conn.execute("INSERT OR IGNORE INTO users (name, points) VALUES (?, 0)"); conn.commit(); conn.close()
    resp = make_response(redirect('/'))
    resp.set_cookie('worm_user', un, max_age=31536000); return resp

@app.route('/admin')
def admin():
    if session.get('user') != 'Sardorbeko008': return "Stop", 403
    conn = get_db()
    movies = conn.execute("SELECT * FROM cinema ORDER BY id DESC").fetchall()
    conn.close()
    ...
    movie_list = ""
    for m in movies:
        movie_list += f'<div style="border-bottom:1px solid #333; padding:10px;">{m["title"]} <a href="/delete/{m["id"]}" style="color:red; float:right;">[X O`CHIRISH]</a></div>'

    return f'''<body style="background:#000;color:#fff;padding:20px;font-family:sans-serif;">
    <h2>ADMIN PANEL - QO`SHISH</h2>
    <form action="/add_movie" method="POST" style="background:#111; padding:15px; border-radius:10px;">
        ...
        Video Link (Telegram Direct Link): <br><input name="s" required style="width:100%; padding:10px; margin-bottom:10px;" placeholder="https://...mp4"><br>
        <button style="background:green; color:white; padding:15px; width:100%; border:none; font-weight:bold;">SAYTGA QO`SHISH</button>
    </form>
    <br><h2>MAVJUD KINOLAR</h2>
    <div style="background:#111; border-radius:10px; padding:10px;">{movie_list}</div>
...
def add_movie():
    if session.get('user') == 'Sardorbeko008':
        ...
        conn = get_db()
        conn.execute("INSERT INTO cinema (title, source) VALUES (?,?)", (t, s))
...
def delete(id):
    if session.get('user') == 'Sardorbeko008':
        conn = get_db()
        conn.execute("DELETE FROM cinema WHERE id = ?", (id,))
        conn.commit(); conn.close()
    return redirect('/admin')
